...
function buildTestApp() {
  // Minimal Express app that mounts validateRequest on POST /investigate.
  // Used only by these tests so we can assert middleware behavior without
  // needing a real /investigate route in the production app.
...
describe('middleware: validateRequest (HTTP)', () => {
  ...
  test('validateRequest middleware does not mutate req.body', async () => {
    // Direct middleware check (no HTTP): we call it via a dummy req/res.
    const validateRequest = require('../src/middleware/validateRequest');
