...
describe('POST /analyze-ticket', () => {
  ...
  test('customer_reply does not ask the customer for their PIN or OTP', async () => {
    ...
    // Split on sentence-ending punctuation so each chunk is examined in
    // isolation. A "do not share your PIN" warning must not be mistaken
    // for a request just because it lives in the same string.
    ...
    for (const sentence of sentences) {
      // If a sentence opens with a negation, treat any later mention of
      // an ask-verb + pin/otp as a safety warning, not a request.
